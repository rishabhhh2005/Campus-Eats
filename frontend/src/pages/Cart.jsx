...
export default function Cart() {
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      // Handle card payment flow: open Razorpay first, create order only after success
      if (paymentMethod === 'card') {
        setRazorAuto({ amount: finalAmount });
  ...
  const handleRazorSuccess = async (verifyRes, orderResponse) => {
    ...
    try {
      ...
      await finalizeAfterSave({ id: response.id }, finalAmount);
    } catch (err) {
  ...
  placeholder=" Use CAMPUS10 for 10% off"
  className="flex-1 border px-4 py-2 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
/>
        ...
        <RazorPayReact
          ...
          onSuccess={async (v, order) => {
            try {
              ...
              await handleRazorSuccess(v, order);
